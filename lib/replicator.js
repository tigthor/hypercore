...
const { CodedRepair, serveCodedRequest, MAX_GROUP } = require('./coded')

const DEFAULT_MAX_INFLIGHT = [16, 512]
const DEFAULT_CODED_GROUP = 16
...
class Peer {
  constructor(replicator, protomux, channel, inflightRange) {
    ...
    this.wireCodedRequest = this.channel.messages[10]
    this.wireCodedData = this.channel.messages[11]

    // coded replication: advertised by the remote in its handshake, driven by
    // the per-peer controller (only constructed when we opted in locally)
    this.remoteCoded = false
    this.coded = replicator.coded ? new CodedRepair(this) : null
    this.codedServing = 0 // concurrent coded encodes we are serving this peer
    ...
    this.stats = {
      ...
      wireCodedRequest: { tx: 0, rx: 0 },
      wireCodedData: { tx: 0, rx: 0 },
      hotswaps: 0,
      invalidData: 0,
      invalidRequests: 0,
      backoffs: 0,
  ...
  onopen({ seeks, coded, capability }) {
    ...
    this.remoteCoded = coded === true && this.coded !== null
  ...
  // Request just the tree hash for `index` (no block body) and attach a ref to
  // `session` (an array) whose promise resolves once the leaf is verified into
  // storage. Used by coded repair to establish trusted leaf hashes before it
  // fills the bodies. Reuses the normal hash-request path, so hypercore's own
  // proof verification does the trusting.
  _requestHashAt(index, session) {
    const h = this.replicator._hashes.add(index, PRIORITY.NORMAL, false)
...
module.exports = class Replicator {
  ...
  constructor(
    core,
    {
      ...
      inflightRange = null,
      coded = false,
      codedGroup = DEFAULT_CODED_GROUP
    } = {}
  ) {
    this.core = core
    ...
    this.coded = coded
    // clamp to the served maximum - an oversized group would have every coded
    // request rejected by a conformant peer
    this.codedGroup = Math.min(MAX_GROUP, Math.max(1, codedGroup | 0))
  ...
  _removePeer(peer) {
    ...
    peer.wants.destroy()
    if (peer.coded !== null) peer.coded.destroy()
  ...
  updatePeer(peer) {
    ...
    if (peer.coded !== null) peer.coded.update()
  ...
  _makePeer(protomux) {
    ...
    const channel = protomux.createChannel({
      ...
      messages: [
        ...
        { encoding: m.wire.codedRequest, onmessage: onwirecodedrequest },
        { encoding: m.wire.codedData, onmessage: onwirecodeddata }
      ],
      onopen: onwireopen,
      onclose: onwireclose,
      ondrain: onwiredrain,
...
function onwirecodedrequest(m, c) {
  incrementRx(c.userData.stats.wireCodedRequest, c.userData.replicator.stats.wireCodedRequest)
  // only serve coded requests if we opted in; an opted-out node never advertises
  // the handshake bit, so a conformant peer never asks, and this keeps a
  // non-conformant one from making us do coded work
  if (c.userData.coded !== null) serveCodedRequest(c.userData, m)
}

function onwirecodeddata(m, c) {
  incrementRx(c.userData.stats.wireCodedData, c.userData.replicator.stats.wireCodedData)
  if (c.userData.coded !== null) c.userData.coded.ondata(m)
}
