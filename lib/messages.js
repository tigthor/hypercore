...
wire.handshake = {
  ...
  encode(state, m) {
    c.uint.encode(state, (m.seeks ? 1 : 0) | (m.coded ? 2 : 0))
...
// Coded replication (optional, negotiated by the handshake `coded` flag).
// A downloader asks a peer for `count` repair symbols covering the group of
// `k` blocks starting at `start`; the peer answers with codedData symbols.
wire.codedRequest = {
  preencode(state, m) {
    c.uint.preencode(state, m.start)
...
// One encoding symbol for the group at `start`. `esi` is the symbol id (esi >=
// k is a repair symbol); `symbol` is the fixed-size coded payload. The block
// lengths and symbol size are derived from the Merkle tree on both sides, so
// they never travel here.
wire.codedData = {
  preencode(state, m) {
    c.uint.preencode(state, m.start)
    c.uint.preencode(state, m.k)
