...
class CodedRepair {
  ...
  async _start(start) {
    ...
    if (this._stale(start, fork)) return this._reset()
    ...
    if (this._stale(start, fork) || symbolSize === 0) return this._reset()
  ...
  // every exit from _start must land here (or in _kick): before the repair
  // phase no timer exists, so a group left behind blocks update() for the rest
  // of the connection
  _reset() {
    this._clearActive()
  }
...
module.exports = { CodedRepair, serveCodedRequest, MAX_GROUP }
